...
class ChopperClient {
  ...
  Future<Response<Value>> decodeResponse<Value>(
    ...
    final Response<Value> converted =
        await _converter?.decode(response, responseType);

    if (converted == null) {
      throw new Exception("No converter found for type $Value");
    }
    print("decodeResponse OK");
  ...
  Future<Response<Value>> send<Value>(Request request,
    ...
    if (!res.isSuccessful) {
      print("Somwething when wrong with reponse");
      throw res;
    }
